...
def process_post_db(db_out):
    A_decoded = db_out[:, 1:, :, :]
    A_mask_alone = db_out[:, :1, :, :].repeat((1, 3, 1, 1))
    colored_mask_to_apply = A_decoded * A_mask_alone
    return colored_mask_to_apply
...
def save_imgs(args, e1, e2, d_a, d_b,stn, iters):
    ...
    for i in range(args.num_display):
        with torch.no_grad():
            if i == 0:
                ...
                exps5.append(filler.fill_(0))
                exps6.append(filler.fill_(0))
            ...
            exps5.append(test_domB[i].unsqueeze(0))
            exps6.append(test_domB[i].unsqueeze(0))
    ...
    for i in range(args.num_display):
        ...
        exps5.append(test_domA[i].unsqueeze(0))
        exps6.append(test_domA[i].unsqueeze(0))
        ...
        for j in range(args.num_display):
            with torch.no_grad():
                ...
                BA_zraw = d_b(BA_encoding, test_domB[j])

                orig_mask_BA = BA_zraw[:,:1,:,:].repeat((1,3,1,1))
                db_output = BA_zraw[:,1:,:,:]
                BA_decoded,_, moved_mask_BA = stn(BA_zraw, test_domB[j], return_mask = True)
                # moved_mask_BA = moved_mask_BA.repeat((1, 3, 1, 1))

                # BA_colored_mask_to_apply = process_post_db(BA_zraw)
                # BA_decoded,BA_colored_mask_to_apply = stn(BA_colored_mask_to_apply, test_domB[j], return_mask=True)


                AA_encoding = torch.cat([common_A, separate_A], dim=1)
                AA_zraw = d_b(AA_encoding, test_domA[j])
                orig_mask_AA = AA_zraw[:,:1,:,:].repeat((1,3,1,1))
                AA_decoded,_ = stn(AA_zraw, test_domA[j])

                # colored_mask_to_apply = process_post_db(AA_zraw)
                # AA_decoded = stn(colored_mask_to_apply, test_domA[j])

                # AA_decoding, mask2, _ = stn(AA_decoding, test_domA[j])
                # # mask2, _dec_sep_output = stn(mask2, _dec_sep_output)

                A_decoding = d_a(AA_encoding)
                exps.append(BA_decoded)
                exps2.append(orig_mask_BA)
                exps4.append(orig_mask_AA)
                exps5.append(moved_mask_BA)
                exps6.append(db_output)
                if j % 2 == i % 2:
    ...
    with torch.no_grad():
        ...
        exps5 = torch.cat(exps5, 0)
        exps6 = torch.cat(exps6, 0)
    # print("Exp max: " + str(exps.max()))
    # print("Exp min: " + str(exps.min()))
    vutils.save_image(exps,
                      '%s/experiments_%06d.png' % (args.out, iters),
                      normalize=True, nrow=args.num_display + 1)
    vutils.save_image(exps2,
                      '%s/masks_seperate_%06d.png' % (args.out, iters),
                      normalize=True, nrow=args.num_display + 1)
    vutils.save_image(exps3,
                      '%s/d_a_%06d.png' % (args.out, iters),
                      normalize=True, nrow=args.num_display + 1)
    vutils.save_image(exps4,
                      '%s/segmentation_common_%06d.png' % (args.out, iters),
                      normalize=True, nrow=args.num_display + 1)
    vutils.save_image(exps5,
                      '%s/BA_moved_mask_%06d.png' % (args.out, iters),
                      normalize=True, nrow=args.num_display + 1)
    vutils.save_image(exps6,
                      '%s/db_output%06d.png' % (args.out, iters),
                      normalize=True, nrow=args.num_display + 1)
