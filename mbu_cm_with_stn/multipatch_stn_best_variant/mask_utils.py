    ...
    for i in range(args.num_display):
        ...
        for j in range(args.num_display):
            with torch.no_grad():
                ...
                BA_zraw, _ = d_b(BA_encoding, test_domB[j])

                orig_mask_BA = BA_zraw[:,:1,:,:].repeat((1,3,1,1))
                BA_decoded, moved_mask_BA = stn(BA_zraw, test_domB[j], return_mask = True)
                moved_mask_BA = moved_mask_BA
                ...
                AA_zraw, _ = d_b(AA_encoding, test_domA[j])
                orig_mask_AA = AA_zraw[:,:1,:,:].repeat((1,3,1,1))
                AA_decoded = stn(AA_zraw, test_domA[j])
