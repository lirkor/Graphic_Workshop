    ...
    while True:
        ...
        for domA_img, domB_img in zip(domA_loader, domB_loader):
            ...
            if args.noise>0:
                ...
                val = uniform(0,1)
                if val<noise_ratio:
                    domA_img = domA_img.roll((15,18),dims=(2,3))
                    domB_img = domB_img.roll((12,17),dims=(2,3))
            ...
            zraw_mask, da_shaved = d_b(A_encoding, d_a(A_shaved_encoding))
            ...
            # move loss
            # loss += args.lambda1 * mse_mask(domA_img, domA_img+A_recon_attr*(A_moved_mask)) + \
            #     args.lambda2 * mse_mask(domB_img, domB_img+B_recon_attr*B_moved_mask)
            ...
            if args.output_disc_weight > 0 and 0:
                pred_c_out = disc_out(C_decoded)
                pred_A_out = disc_out(A_decoded)
                loss += args.output_disc_weight * (bce_ct(pred_c_out, A_label) + bce_ct(pred_A_out, A_label))
            ...
            if args.output_disc_weight > 0:
                ...
                loss = bce_ct(real_no_glasses_domain, A_label) + \
                       bce_ct(real_with_glasses_domain, A_label)\
