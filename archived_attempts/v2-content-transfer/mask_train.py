...
import torchvision
from mask_models import E1, E2, D_A, Disc, D_B, STN, V2_STN
...
def train(args):
    ...
    noFlowTensor = torch.tensor(np.zeros((args.resize, args.resize, 2)), dtype=torch.float)
    ...
    while True:
        ...
        for domA_img, domB_img in zip(domA_loader, domB_loader):
            ...
            A_decoding, _ = d_b(A_encoding, d_a(A_shaved_encoding), domB_img, stn, A_shaved_encoding)
            ...
            C_decoding, _ = d_b(C_encoding, domB_img, domB_img, stn, A_shaved_encoding)

            B_rec, _ = d_b(B_encoding, domB_img, domB_img, stn, A_shaved_encoding)
            A_rec, _ = d_b(A_encoding, domA_img, domB_img, stn, A_shaved_encoding)
            ...
            if _iter % args.save_iter == 0 and _iter > 0:
                ...
                save_model(save_file, e1, e2, d_a, d_b, stn, ae_optimizer, disc, disc_optimizer, _iter)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--discweight', type=float, default=0.01)
    parser.add_argument('--thetaweight', type=float, default=0.002)
